...
[English](public-contract-v1.md) | **日本語**

この文書はstable `v1.x`で維持する公開互換性の意味を定義します。内部実装の全てをpublic APIにするものではありません。
...
portable schema evolution、migration、baseline authenticity、deprecation ruleは[Schema evolution v1](schema-evolution-v1.ja.md)で定義します。
...
adapter identity、maturity、core PASS、capability、protocol-era meaningは[Interoperability semantics v1](semantic-contract-v1.ja.md)で定義します。

security、privacy、credential isolation、cleanup、privileged CI、tagged-release guaranteeは[Security, privacy, cleanup, and release contract v1](security-contract-v1.ja.md)で定義します。
