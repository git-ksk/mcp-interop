...
Suite manifest v1は、v0.7の繰り返し検証で使う宣言形式です。`suite` CLIは内容を検証し、`trusted_real_client` manifestを実行できます。結果の比較方法は[Suite regression report v1](suite-regression-report-v1.ja.md)で定義済みです。`hosted_fixture`宣言はv0.9.0でもvalidation-onlyで、repository CIのcontrolled localhost fixtureは別の信頼済み経路で実行します。
