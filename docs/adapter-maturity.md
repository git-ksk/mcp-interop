...
The pre-v1 review promotes **Codex, Cursor, and Antigravity to stable for their explicitly documented macOS arm64 non-OAuth core paths**. Adapter maturity is an evidence review, not a regression result and not an automatic consequence of a version change.
...
| Cursor CLI | `stable` | isolated supported MCP management surface; same non-OAuth core PASS path on exact `2026.08.04-aaa8809` and `2026.09.02-c22c1a3` on macOS arm64; retained `2026.08.25-3e8eec8` core evidence; safety/regression gates | none for the advertised stable scope. OAuth and non-macOS/macOS amd64 remain outside the stable claim. |
| Antigravity CLI | `stable` | isolated PTY/no-account boundary; same non-OAuth core PASS path on exact `1.1.22` and `1.1.24` on macOS arm64; repeated bounded tool-cache observation; safety/regression gates | none for the advertised stable scope. OAuth, non-macOS, and modern `server/discover` tool-discovery success remain outside the stable claim. |
